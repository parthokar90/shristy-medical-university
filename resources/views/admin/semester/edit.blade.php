        ...
        <div class="row">
          ...
          <div class="col-md-12">
            ...
            <div class="card card-primary">
              ...
              <!-- /.card-header -->
              <div class="card-body">
              <form role="form" method="POST" action="{{route('semesters.update',$edit->id)}}">
                    {{csrf_field()}}
                    {{METHOD_FIELD('PATCH')}}
                  <div class="row">
                    ...
                    <div class="col-sm-4">
                        <div class="form-group">
                            ...
                            <input type="text" name="semister_name" id="semister_name" class="form-control" placeholder="Enter Semester Name" value="{{$edit->semister_name}}" autocomplete="off">
                        </div>
                    </div>
                            ...
                            <label>Selected Course</label>
                                <select class="form-control" name="course_id[]" multiple>
                                  ...
                                  @foreach($course as $courses)
                                   @php 
                                      $c=DB::table('semister_courses')->where('semister_id',$edit->id)->where('course_id',$courses->id)->first(); 
                                   @endphp
                                   @if(isset($c)) 
                                     @php $cc=$c->course_id; @endphp
                                   @endif
                                    <option value="{{ $courses->id }}"      {{ $cc == $courses->id ? 'selected="selected"' : '' }}             >
                                        {{ $courses->course_name }}
                                    </option>
                                  @endforeach  
                                 </select>
